...
实测环境是**交互用户会话**（登录的桌面）：GSMTC 元数据、传输控制、WASAPI loopback 采集全部正常。

把它包成 Windows 服务、或在**会话 0**（非交互）里运行时是否还能看到媒体会话，**没有验证** ——
试过用 PowerShell 从会话 0 调 WinRT 直接探测，但 PowerShell 5.1 的 WinRT 异步调用会挂住
（会话 1 里用同样的写法也挂），所以那是 PowerShell 的问题、不构成"会话作用域"的证据，
这里就不下结论。宿主若要把它放进服务/后台进程，先用一条命令确认：

```powershell
# 在登录的桌面里放一首歌，然后在**目标上下文**里跑：
media-bridge now      # 能读到曲目 → 该上下文可见媒体会话；返回"没有正在播放的媒体" → 不行
```

（Linux 的 MPRIS 走 D-Bus 会话总线，天然要求同一用户会话；macOS 的 MediaRemote 同理。
也就是说三平台都更适合「跟着用户会话跑」的宿主形态。）
